...
class SCIPSVMBase:
    ...
    def addProblemVariables(self):
        ...
        for f in xrange(self.nfeatures + 1):
            self.omegas.append(self.model.addVar(vtype='C', name="omega_%d" % f, ub=self.weightBound, lb = -self.weightBound))
        
        
        self.xis = []
        # add variables xi to penalize misclassification
        for x in xrange(self.nexamples):
                self.xis.append(self.model.addVar(vtype='C', name="psi_%d" %x))
    ...
    def addObjectiveFunction(self, X, y):
        ...
        self.artobjvar = self.model.addVar(vtype='C', name="art_obj", obj=1.0)
        
        # apply correction to account for unequal proportions of positive and negative example
        objCoeffs = [self.classWeights[0 if yi == -1 else 1] for yi in y]
        
        name="objective_function"
    
        self.model.addCons(quicksum(.5 *self.omegas[f]*self.omegas[f] for f in xrange(self.nfeatures)) \
                           + quicksum(self.C / float(self.nexamples) * objCoeffs[x] * self.xis[x] for x in xrange(self.nexamples)) - self.artobjvar <= 0, name=name)
...
class SCIPSVMLinear(SCIPSVMBase):
    ...
    def addModelFormulation(self, X, y):
        for x in xrange(self.nexamples):
            self.model.addCons(quicksum(y[x] * X[x][f] * self.omegas[f] for f in xrange(self.nfeatures)) + y[x] * self.omegas[self.nfeatures] >= + 1 - self.xis[x], name="example_%d"%x)
...
class SCIPSVMSparseLinear(SCIPSVMBase):
    ...
    def addModelFormulation(self, X, y):
        ...
        for f in xrange(self.nfeatures):
            vs.append(self.model.addVar(vtype='B', name="v_%d" % f))
            self.model.addCons(self.omegas[f]  <= self.weightBound * vs[f], name="upper_vbound_%d"%f)
            self.model.addCons(self.omegas[f]  >= -self.weightBound * vs[f],name="lower_vbound_%d"%f)
            
        self.model.addCons(quicksum(vs[f] for f in xrange(self.nfeatures)) <= int(self.sparsity * self.nfeatures), name="sparsity")